...
const int iServoEnable_Pin = 5;     // Brake Servo N-FET Power Enable Pin

// Load Setting Variables 
int iLoadSetting = 0;                 // Load Setpoint Value
byte bLoad12VEnabled = 0;             // Is +12V Load Power enabled?

// Brake Variables
int iBrakeSetting = 1000;             // Brake Setting
byte bServoEnabled = 0;               // Servo Power Enabled Boolean Variable
...
void loop() {
  ...
  const int iRXSize = 57;               // Serial Input Length (Characters from Load)
  ...
  while(!bRXTimeout && !bRXComplete) {
    ...
    if(bStartCharFound && Serial.available() >= iRXSize-1) {
      ...
      bServoEnabled = Serial.parseInt();    // Is the Servo Enabled?
      bEStop_Last = Serial.parseInt();      // Last E-stop Value
      bLoad12VEnabled = Serial.parseInt();  // Is the load +12V power enabled?      
      bRXComplete = true;
    }
  ...
  /******************************************************************/
  // Before doing anything, measure the E-stop state
  bEStop_Last = bEStop_Now;             // Store the last button state
  bEStop_Now = digitalRead(iEStop_Pin); // Will return 1 if the button is closed (no Emergency) and 0 if open (Emergency)
  // Do something if the E-stop changes state
  if(bEStop_Now < bEStop_Last) {
    // If the state changes from 1 to 0, activate emergency stop protocol
    iBrakeSetting = 2000;
    myservo.writeMicroseconds(iBrakeSetting);    //change whatever number is in () to move actuator
    bServoEnabled = 1;
    digitalWrite(iServoEnable_Pin, HIGH);
  }
  if(bEStop_Now > bEStop_Last) {
    // If the state changes from 0 to 1, remove the emergency stop condition
    iBrakeSetting = 1000;
    myservo.writeMicroseconds(iBrakeSetting);    //change whatever number is in () to move actuator
    bServoEnabled = 1;
    digitalWrite(iServoEnable_Pin, HIGH);
  ...
  // the servo as indicated from the received datat from the Load
  if(bEStop_Now == 1) {
    if(bServoEnabled == 1) { 
      myservo.writeMicroseconds(iBrakeSetting);
      digitalWrite(iServoEnable_Pin, HIGH); 
    }
    if(bServoEnabled == 0) {
